...
const ProgramCard: React.FC<ProgramCardProps> = ({ program, studentData }) => {
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    ...
    // Navigate to detail page - encode state in URL params
    const params = new URLSearchParams({
      programData: JSON.stringify(program),
      studentData: studentData ? JSON.stringify(studentData) : ''
    });
    router.push(`/program/${encodeURIComponent(program.program_name)}?${params.toString()}`);
  };
          ...
          <div className="mb-5">
            ...
            <div className="space-y-2">
              {todoItems.map((item, idx) => (
                <CourseChip key={idx} course={item} />
              ))}
            </div>
          </div>
        ) : isCompleted ? (
