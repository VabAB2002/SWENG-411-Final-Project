...
import { PrerequisiteTreeProps, CoursesData } from '@/types';

interface Prerequisite {
  code: string;
  normalized: string;
  status: 'met' | 'available' | 'notMet';
  raw?: string;
}

const PrerequisiteTree: React.FC<PrerequisiteTreeProps> = ({ 
  courseCode, 
  userHistory = [], 
  coursesData = {},
  level = 0 
  ...
  const parsePrerequisites = () => {
    ...
    const courseData = coursesData[courseCode.replace(/\s+/g, '')];
  ...
  const checkPrereqStatus = (courseCode: string, history: string[]): 'met' | 'available' | 'notMet' => {
  ...
  const toggleNode = (courseCode: string) => {
  ...
  const getNestedPrerequisites = (courseCode: string): Prerequisite[] => {
  ...
  const renderPrereqNode = (prereq: Prerequisite, depth: number = 0): React.ReactNode => {
        ...
        <AnimatePresence>
          {isExpanded && nestedPrereqs.length > 0 && (
            <motion.div
              ...
              transition={{ duration: 0.3 }}
            >
              {nestedPrereqs.map((nested) => 
                renderPrereqNode(nested, depth + 1)
              )}
            </motion.div>
          )}
        </AnimatePresence>
